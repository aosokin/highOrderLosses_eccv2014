...
%
%   Modified by Anton Osokin in 2012
%
*=================================================================*/
...
#include <limits> //AOSOKIN
#include <math.h> //AOSOKIN
...
// AOSOKIN
double colorDistancePower = 2.0;
double colorDistanceMax = std::numeric_limits<double>::max();
        ...
        if( ii>=0 && jj>=0 && ii<n && jj<p)
        {
          ...
          if(entryOk)
          {
            ...
            for(iCh=0;iCh<nCh;iCh++){
				...
				//AOSOKIN
				double curDiff = W_(ii,jj,iCh)-W_(i,j,iCh);
				if	(fabs(colorDistancePower - 2.0) < 1e-10) {
					curDiff = SQR(curDiff);
				}
				else {
					// no more l2
					curDiff = pow(fabs(curDiff), colorDistancePower);
				}
				// truncate curDiff
				curDiff = (curDiff < colorDistanceMax ) ? curDiff : colorDistanceMax;
				l2_gradient += curDiff;
				 ...
				 int nrhs, const mxArray*prhs[] ) 
{
  ...
  // AOSOKIN
  if (nrhs > 10) {
	  colorDistancePower = (double) *mxGetPr(prhs[10]);
  }
  if (nrhs > 11) {
	  colorDistanceMax = (double) *mxGetPr(prhs[11]);
  }
