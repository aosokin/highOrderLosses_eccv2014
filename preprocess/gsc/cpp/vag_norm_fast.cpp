...
void displayDims( const char *name, const size_t *dims, int ndims ) {
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  */
  ...
  const size_t *nX = mxGetDimensions( prhs[ 0 ] );
  const size_t *nMU = mxGetDimensions( prhs[ 1 ] );
  const size_t *nIS = mxGetDimensions( prhs[ 2 ] );
  const size_t *nK = mxGetDimensions( prhs[ 3 ] );
