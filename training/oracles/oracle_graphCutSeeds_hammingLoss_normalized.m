function [F, C, warmStart] = oracle_graphCutSeeds_hammingLoss_normalized(iObject, Y, W, config)
%oracle_graphCutSeeds_hammingLoss_normalized - oracle for SSVM: loss augmented MAP-inference;
% binary segmentation, pairwise MRF, Potts pairwise weights, graphcut, user-defined seeds
% Hamming loss normalized to have maximum of 1
...
%
% [F, C, warmStart] = oracle_graphCutSeeds_hammingLoss_normalized(iObject, Y, W, config);
%
% INPUT
...
[variables, flagAdded] = loadVariableGlobalDataset('normalizationFactorHamming', iObject, config.loadDataInMemory);
if flagAdded
    normalizationFactor = variables{1};
else
    normalizationFactor = sum( objSeed(:) == 0 & bkgSeed(:) == 0 & ~isnan(Y(:)));
    addVariableGlobalDataset('normalizationFactorHamming', iObject, normalizationFactor);
end
...
checkDiff = abs(-energy_worst - (F' * W + C));
if  checkDiff > 1e-4 * config.epsNormalization
